...
def test_manual_trips_table_exists(db):
    cursor = db.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='manual_trips'"
    )
    assert cursor.fetchone() is not None
...
def test_category_rules_muni_metro(db):
    ...
    row = db.execute(
        ...
        " WHERE operator = 'Muni' AND location = 'Embarcadero'"
    ).fetchone()
...
def test_category_rules_cable_car(db):
    ...
    row = db.execute(
        "SELECT category FROM category_rules"
        " WHERE operator = 'Muni' AND location = 'Hyde/Beach'"
    ).fetchone()
...
def test_trip_id_unique_index_allows_null(db):
    ...
    db.execute(
        "INSERT INTO trips (account_number, start_datetime, operator)"
        " VALUES ('A', '2024-01-01', 'BART')"
    )
    db.execute(
        "INSERT INTO trips (account_number, start_datetime, operator)"
        " VALUES ('A', '2024-01-02', 'BART')"
    )
    db.commit()
...
def test_trip_id_unique_index_prevents_dupes(db):
    db.execute(
        "INSERT INTO trips (account_number, trip_id, start_datetime, operator)"
        " VALUES ('A', 'T1', '2024-01-01', 'BART')"
    )
    db.commit()
    with pytest.raises(sqlite3.IntegrityError):
        db.execute(
            "INSERT INTO trips (account_number, trip_id, start_datetime, operator)"
            " VALUES ('A', 'T1', '2024-01-02', 'BART')"
        )
